...
def shape_function_spatial_derivative(V, triangle_encoding, xi, x, n):
    """
    Computes the spatial derivatives of the all the shape function with order n.
    The V parameter contains the (x,y) pairs of each vertex on the mesh.
    ...
    barycentric_coordinates = ijk_indices_e / n
    ...
    dN_dxi = np.zeros((m, 3))
    for i in range(ijk_indices_e):
        for xi_index in range(3):
            xi = barycentric_coordinates[i][xi_index] # Taking the derivative with respect to xi_{xi_index}
